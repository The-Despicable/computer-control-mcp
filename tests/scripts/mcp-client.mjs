...
export class Mcp {
  ...
  async handshake() {
    const r = await this.request("initialize", {
      ...
      capabilities: {}, clientInfo: { name: "gate-harness", version: "1.0.0" },
    });
  ...
  stop() { try { if (this.proc) this.proc.kill(); } catch { /* already dead */ } }
}
...
export function errCode(res) {
  const p = parseToolResult(res);
  if (p.json && typeof p.json === "object" && "error" in p.json) {
    const e = p.json.error;
...
export function textJson(res) {
  return parseToolResult(res).json ?? {};
}

export const PASS = (n) => console.log(`PASS ${n}`);
export const FAIL_ = (n, why = "") => { console.error(`FAIL ${n} ${why}`); process.exitCode = 1; };
export const spawnPowershell = (script) =>
  new Promise((res, rej) => {
    const p = spawn("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script]);
    let o = "";
    if (p.stdout) p.stdout.on("data", (d) => { o += d.toString(); });
    p.on("close", (c) => { if (c === 0) res(o.trim()); else rej(new Error(o)); });
    p.on("error", rej);
  });
